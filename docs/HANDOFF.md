...
> 마지막 업데이트: 2026-07-16 (**classic.html v2 r27** — 씬 앵커 드리프트: 스크롤 멈추면 다음 장면-완성 앵커까지 페이지가 스스로 글라이드. 게임 모드는 r5 상태 유지). yoonkihong.com 리빌드 프로젝트의 전체 인수인계 문서.
...
- **씬 앵커 드리프트 (r27, r28 감속, r29 미세조정)**: 스크롤을 멈추면(IDLE_MS 550ms) 페이지가 **실제 스크롤 위치를** DRIFT_RATE 0.0325 p/s(r27 0.05 "너무 빨라" → r28 0.025 → r29 +30% = 0.0325, 사용자 튜닝)로 다음 앵커까지 글라이드 후 정지 — 앵커 = [서울 완성(=서울 dwell 시작 p, pForT로 easeFilm 역산), SF 완성, 락인 타블로 p 0.7(AUTO_FROM 통과 → 필름은 오토플레이가 담당, 페이지는 헤드라인+CTA를 올림)]. 진짜 scrollTo를 움직이므로 필름/비트/스크림/레일이 공짜로 동기화(드리프트된 필름 vs 스크롤바 조정 문제 자체가 없음). 휠/터치/키 입력 즉시 취소(사용자 우선), 우리가 쓴 scrollTo는 `expectY ±3px` 매치로 사용자 입력과 구분. 발동 게이트: p ∈ (0.03, 0.688), 서울/SF dwell 안(=앵커 위)이나 모달 열림·hidden·DBG_P 핀 상태에선 발동 안 함
- **iOS 하단 여백 수정 (r28)**: `.hero-sticky`/`.products-sticky`가 `100svh`(작은 뷰포트 고정)라서 사파리 URL 바가 접히면 sticky 아래로 페이지 배경 띠 노출 — 드리프트로 "손 안 대고 보는" 시간이 길어지며 드러남. `height:100vh`(폴백)+`100dvh`로 교체, 바 개폐 시 resize 이벤트로 캔버스 fit() 재실행됨. **svh로 되돌리지 말 것**
...
- **프리뷰 패널 심화 제약 (r27에서 확인)**: 패널 탭은 `document.hidden=true`(visibilityState 'hidden')이고, **프로그램적 scrollTo로 scrollY가 바뀌어도 scroll 이벤트가 디스패치되지 않음** — scroll 리스너 기반 기능(앵커 드리프트 등)은 패널에서 자연 발동 불가. 검증 레시피: javascript_tool은 페이지 월드에서 실행되므로 ① `Object.defineProperty(document,'hidden',{get:()=>false})` + rAF→setTimeout 심 주입, ② `scrollTo(...)` 후 `dispatchEvent(new Event('scroll'))`로 체인 수동 트리거, ③ scrollY 폴링으로 글라이드/정지 확인 (hidden 탭 타이머는 1s로 클램프되어 실제보다 느리게 진행됨)
