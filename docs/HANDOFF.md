...
> 마지막 업데이트: 2026-07-10 (폴리시 r4 + 애널리틱스 + 커리어 콘텐츠 배포 후). yoonkihong.com 리빌드 프로젝트의 전체 인수인계 문서.
...
- **최신 배포 상태**:
  - product 6종(macrodoc, mathstreet, mathwings, funnify, lasthand, gunball) + egg 3종
  - 플레이어/NPC = 리깅+애니메이션 GLB, 플레이어는 **파타고니아풍 플리스**로 재수출(§5)
  - 맵 = 허브-스포크 마을(분수 광장 중심 + 2건물 지구 3곳 + 남쪽 nursery + SE Demo Lab — `const.js` RAW_MAP 주석)
  - 포켓몬식 선택지 메뉴(§6), 신나는 16비트 BGM(§5)
  - 집 내부 커리어 룸 = **실제 커리어 내용**(NAVER 해외사업 공채→LINE 사업부, LINE 동남아 그로스+신규서비스, Google Play 한/미·Promotional Content·AI agentic content system) — interior.js 명판 + NPC CAREER + classic.html Experience 3곳 모두 동기화
  - OG/트위터 셰어 카드(`images/og.jpg`), profile 사진 경량화, PostHog+Sentry 애널리틱스(§11)
...
3. **ElevenLabs `music_generation` 권한 추가 시 BGM을 정식 `/v1/music`으로 재생성** (2026-07-10 재시도도 401 — 현재는 sound-generation 우회물, §5)
